...
let product1 = new Product(
  '../IMG/perfume-bottle-bergamot.jpg',
  'Bergamot',
  850,
  description,
  1,
...
let product3 = new Product(
  '../IMG/perfume-bottle-saffron.jpg',
  'Saffron',
  850,
  description,
  3,
...
let product4 = new Product(
  '../IMG/perfume-bottle-sandalwood.jpg',
  'Sandalwood',
  850,
  description,
  4,
...
$(function () {
  ...
  if (localStorage.getItem('products') != null) {
    //Only get bag from LS if it has content
    getBag();
  }
  ...
  $.each(myProducts, (i, product) => {
    ...
    $('<p>').appendTo(textLeft).html(product.name);
...
function createBagHTML() {
  ...
  $.each(bag, (i, product) => {
    ...
    $('<button>')
      ...
      .on('click', { product: product }, function () {
        for (let i = 0; i < bag.length; i++) {
          // Looping bag, if clicked object id is found in bag -> decrease quantity of that product

          if (bag[i].id === product.id) {
            bag[i].quantity--;
            if (bag[i].quantity === 0) {
              // If the quantity of the product becomes 0 -> splice that product from array
              bag.splice([i], 1);
            }
            saveBag();
            getBag();
